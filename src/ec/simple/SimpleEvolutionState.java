...
import poc.gp.ecj.RankAggregationProblem;
...
public class SimpleEvolutionState extends EvolutionState {
    ...
    public void startFresh() {
        ...
        String resamplingInterval = (String) parameters.get("resamplingInterval");
        String sampleSize = (String) parameters.get("sampleSize");
        String sampling = (String) parameters.get("sampling");


        if (sampling == null || sampling.equals("n"))
            output.message("No sampling");
        else if (sampling.equals("sr")) {
            output.message("Using sampling with replacement");
            output.message("\tResampling Interval: " + resamplingInterval + " generations");
            output.message("\tSample Size: " + sampleSize);
        }
...
//        for (Ranking x : Rank.rankings) {
//            if (x.getRankings().stream().anyMatch(userData2 -> userData2.userFitness > 0.0f)) {
//                System.out.println("Fitness greater than 0");
//            }
//        }
        ...
        if (sampling != null && !sampling.equals("n")) {
            String value = (String) parameters.get("resamplingInterval");
            Integer resamplingInterval = Integer.parseInt(value);

            if ((generation + 1) % resamplingInterval == 0 && generation != 0 && generation != numGenerations - 1) {

                ArrayList<Integer> retain = null;

                if (sampling.equals("m3")) {
                    Integer sampleSize = Integer.parseInt((String) parameters.get("sampleSize"));
                    ArrayList<Map.Entry<Integer, Double>> x = new ArrayList<>(RankAggregationProblem.teste.entrySet());
                    ...
                    for (int i = 0; i < sampleSize / 4; i++) {
                        retain.add(x.get(i).getKey());
                    }
                    IO.retain = retain;
                }

                Rank.resetRankingSource();
                output.message("Resampling =D");
            }
        }
